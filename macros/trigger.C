...
void trigger(char *filename,int n=1, int canale=4)
{
  TCanvas *canvas;
  TH1F *h_scope;
  ...
  int tr = trigger(evento,params);
  cout << "baseline: " <<  evento.baseline << endl;
  cout << "offline trigger: " << tr << endl;
  cout << "valore al punto trigger: " <<  evento.samples[tr] << endl;
